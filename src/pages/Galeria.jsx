...
const Galeria = () => {
  const [activeSector, setActiveSector] = useState(null); // Sector visible
  const [activeImage, setActiveImage] = useState(null); // Imagen seleccionada para el carrusel
  const [imageIndex, setImageIndex] = useState(0); // Índice de la imagen activa en el carrusel
  ...
  // Función para abrir el carrusel con una imagen seleccionada
          ...
          <div key={index} className="sector">
            ...
            {activeSector === sector && (
              <div className="imagenes">
                {sectores[sector].map((img, idx) => (
                  <img
                    key={idx}
                    src={img}
                    alt={`${sector} ${idx + 1}`}
                    onClick={() => openCarousel(sector, idx)} // Abrir carrusel al hacer clic
                  />
                ))}
              </div>
            )}
